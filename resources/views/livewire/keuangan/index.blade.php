<div>
    ...
    <div class="p-3">
        <div class="col-md-12 d-flex justify-content-between">
            @if (Auth::user()->hasManagementRoleForKeuangan())
                <x-button class="mb-2 button-add">
                    <a href="{{ route('keuangan.create') }}">Tambah Keuangan</a>
                </x-button>
            @endif
        ...
        <p>Total Selisih : {{ $totalSelisih }}</p>
        <div class="pb-5">
            <div class="p-2 border shadow-table rounded-3">
                <div class="pt-2 col-md-12">
                    <div class="mb-3 d-md-flex d-sm-block justify-content-md-between">
                        <div class="col-md-8 mb-sm-2 text-start">
                        ...
                        <div class="col-md-4 text-sm-start text-md-end">
                ...
                <div class="pb-3 col-md-12">
                    <table class="table table-sortable table-bordered ">
                        <thead class="">
                            <tr>
                                <th class="sort">#</th>
                                <th class="sort">Kegiatan</th>
                                <th class="sort">Pemasukan</th>
                                <th class="sort">Pengeluaran</th>
                                <th class="sort">Selisih</th>
                        ...
                        <tbody>
                            @foreach ($cashes as $key => $cash)
                                <tr wire:key="{{ $cash->id }}">
                                    <td>{{ $cashes->firstItem() + $key }}</td>
                                    <td>{{ ucwords($cash->activity->judul) }}</td>
                                    <td>{{ $cash->pemasukan_formatted }}</td>
                                    <td>{{ $cash->pengeluaran_formatted }}</td>
                                    <td>{{ $cash->total_formatted }}</td>
                                    <td>{{ \Carbon\Carbon::parse($cash->tanggal)->format('d/m/Y') }}</td>
                                    <td>
                                        {{-- button detail --}}
                                        <button class="mr-1 mb-md-1 button btn-detail">
                                            <a href="{{ route('keuangan.show', $cash->id) }}">
                                        ...
                                        @if (Auth::user()->hasManagementRoleForKeuangan())
                                            ...
                                            <button class="mr-1 mb-md-1 button btn-update">
                                                <a href="{{ route('keuangan.edit', $cash->id) }}">
                                            ...
                                            <button class="mr-1 mb-md-1 button btn-delete " data-confirm-delete="true"
                                                wire:click="delete_confirmation({{ $cash->id }})"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Yakin akan menghapus data Keuangan yang dipilih?
                </div>
